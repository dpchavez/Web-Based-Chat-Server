  ...
  for (i = 0; (i <= p->maxi) && (p->nready > 0); i++) {
    ...
    if ((connfd > 0) && (FD_ISSET(connfd, &p->ready_set))) {
      ...
      if ((n = read(connfd, &ps->requestbuf[offset], 8192 - offset)) != 0) {
        ...
        request_type temp = parse_request(ps);
        ...
        if(temp == incomplete)
        {
          ...
          writen(connfd, stream, strlen(stream));
            ...
            //for loop to loop though all file descriptor and find which one is listening
            for(int j = 0; j < p->maxi; j++)
            {
                if (p->receiving_events[j] == 1 && p->clientfd[j] != -1)
                {
